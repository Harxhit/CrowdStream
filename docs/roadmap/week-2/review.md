...
* [x] **Day 4** — Time-limited HMAC TURN credentials wired into the client
...
* [x] Static TURN credentials removed from the client
* [x] Short-lived HMAC TURN credentials generated by the backend
* [x] Coturn configured to authenticate using shared-secret (`use-auth-secret`) credentials
* [x] Coturn rejects expired HMAC credentials (verification pending)
...
* Replaced static TURN credentials with short-lived HMAC credentials generated per authenticated session.
* Integrated backend-issued TURN credentials into the frontend, removing all hardcoded TURN usernames and passwords from client code.
* Configured Coturn to authenticate clients using shared-secret (`use-auth-secret`) HMAC authentication.
...
* Integrating TURN authentication required understanding Coturn's shared-secret authentication flow and coordinating backend-generated credentials with browser WebRTC transports.
...
* Implemented session-based HMAC TURN credentials using Coturn's shared-secret authentication (`use-auth-secret`), replacing static TURN credentials previously used by the client.
