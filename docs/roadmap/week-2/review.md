...
* [x] **Day 2** — JWT verified on the Socket.IO handshake; unauthenticated connections rejected
...
* [x] Connection without a valid access-token cookie is rejected before any signaling handlers run
...
* Secured the Socket.IO handshake by verifying JWTs before any signaling handlers are registered.
* Added authentication middleware that populates `socket.data.user` for authenticated connections.
...
* Socket.IO authentication is complete and uses the existing HTTP-only cookie-based authentication during the handshake.
* Automatic token refresh and token revocation are intentionally out of scope for the MVP.
* TURN HMAC credentials remain scheduled for the next security milestone.
